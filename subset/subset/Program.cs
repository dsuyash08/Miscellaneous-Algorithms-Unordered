    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int k = 0; k < int.Parse(input[0]); k++)
                inputs[k, int.Parse(input[1])] = Convert.ToDecimal(temp2[k]) + Convert.ToDecimal(.0000001);
            ...
            List<int[]> subsets = get(int.Parse(input[1]), int.Parse(input[0]) + int.Parse(input[1]) + 1);
            decimal[] matrixResult1 = new decimal[int.Parse(input[1]) + 1];
             for(int j = subsets.Count - 1; j >= 0; j--)
             {               
                decimal[,] temp = createArrFromSubset(subsets[j], inputs, int.Parse(input[1]));
                ...
                if (matrixResult != null)
                {
                    ...
                    int check = 0;
                    for (int i = 0; i < subsets[j].Length; i++)
                    {
                        if (subsets[j][i] == 0)
                        {
        ...
        static void transform(decimal[,] inputs, int i, int input, int k, int cols )
        {
            for (int j = 0; j < input; j++)
            {
                ...
                {
                    inputs[j, l] = inputs[j, l] - inputs[k, l] * temp;
                }
            }
        }
